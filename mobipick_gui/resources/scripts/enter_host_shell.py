...
def _select_home(home_hint: str | None) -> tuple[Path, Path | None]:
    ...
    # If the hinted directory cannot be inspected we still attempt to use it –
...
def _ensure_rc_stub(home: Path, rc_source: Path | None, uid: int, gid: int) -> None:
    ...
    try:
        ...
        lines = [
            ...
            "mobipick_restore_home=\"$HOME\"",
            f"if [ -f {rc_quoted} ]; then",
        ]

        rc_home = rc_source.parent
        if rc_home != home:
            rc_home_quoted = shlex.quote(str(rc_home))
            lines.extend(
                [
                    f"    HOME={rc_home_quoted}",
                    f"    . {rc_quoted}",
                    "    HOME=\"$mobipick_restore_home\"",
                ]
            )
        else:
            lines.append(f"    . {rc_quoted}")

        lines.extend(
            [
                "fi",
                "HOME=\"$mobipick_restore_home\"",
                "unset mobipick_restore_home",
            ]
        )
